...
def do_db2(sql):
    ...
    db = pymysql.connect(
        user='source',  # 用户名
        password='Lbe87758258',  # 密码：这里一定要注意123456是字符串形式
        host='16.162.114.9',  # 指定访问的服务器，本地服务器指定“localhost”，远程服务器指定服务器的ip地址
...
sql = "select video_id,url from ms_shortvideo where url like '%/shortvideo/%' and origin='lutube'"
res = do_db(sql)

if(res['num']>1):
    for row in res['cursor']:
        print(row[0])
        sql2 = f"select video_id,url from ms_shortvideo where video_id='{row[0]}'"
        print(sql2)
        res2 = do_db2(sql2)
        if(res2['num']>=1):
            ...
            video_id = row[0]
            url = row[1]
            update_sql = f"update ms_shortvideo set url='{url}' where video_id = '{video_id}';"
            with open('C:/shortvideo.sql', "a", encoding="utf-8") as file:
                file.write(update_sql + "\n")
        else:
            with open('C:/error.txt', "a", encoding="utf-8") as file:
                file.write(sql2 + "\n")
            print('not found')